...
fun RowWithProgress(
    ...
    var buttonText by remember { mutableStateOf("Comenzar") }
    ...
    Row(
        ...
        IconButton(
            onClick = {
                if(!progressVisible){
                    onStartScan()
                    Toast.makeText(context,"Iniciando escaneo", Toast.LENGTH_SHORT).show()
                }else{
                    onStopScan()
                }

                progressVisible = !progressVisible
                buttonText = if (progressVisible) "Detener" else "Comenzar"
